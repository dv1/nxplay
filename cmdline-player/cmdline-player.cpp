...
int main(int argc, char *argv[])
{
...

	{
		...
		callbacks.m_duration_updated_callback = [](gint64 const p_new_duration, nxplay::position_units const p_unit)
		{
			switch (p_unit)
			{
				case nxplay::position_unit_nanoseconds:
					std::cerr << "Current duration in ms: " << (p_new_duration / GST_MSECOND) << "\n";
					break;
				case nxplay::position_unit_bytes:
					std::cerr << "Current duration in bytes: " << p_new_duration << "\n";
					break;
			}
		};
		callbacks.m_position_updated_callback = [](gint64 const p_new_position, nxplay::position_units const p_unit)
		{
			switch (p_unit)
			{
				case nxplay::position_unit_nanoseconds:
					std::cerr << "Current position in ms: " << (p_new_position / GST_MSECOND) << "\n";
					break;
				case nxplay::position_unit_bytes:
					std::cerr << "Current position in bytes: " << p_new_position << "\n";
					break;
			}
		};
